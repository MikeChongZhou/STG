...
| `app_background` | App 切到后台 | App Background | 移动端 App 切到后台 | 仅移动端 |
...
| `devices` | string[] | ✅ | [] | 数组长度 ≥ 0 | 涉及的设备 ID 列表 |
...
```json
{
  ...
  "totalSeconds": 29700,
  "sessionCount": 15,
  "devices": ["dev-001-windows-xyz", "dev-002-iphone-abc"],
...
| Android Phone | `{品牌} {型号}` | Xiaomi 14 |
| Android Pad | `{品牌} {型号}` | Samsung Galaxy Tab S9 |
...
| `syncFolderPath` | string | ❌ | null | 合法文件系统路径 | 网盘同步文件夹路径 |
...
平台实现：
  ...
  - macOS：
    ...
    3. 菜单栏图标 tooltip："ScreenGuarding V1.1"
